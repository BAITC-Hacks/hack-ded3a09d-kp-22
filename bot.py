...
STOPWORDS = {
    ...
    # вопросительные слова — есть почти в каждом вопросе, не несут темы
    "когда", "куда", "где", "кто", "какой", "какая", "какие", "какое",
    "сколько", "чем", "зачем", "почему",
}
...
def keywords(text):
    ...
    return {w[:3] for w in words if w not in STOPWORDS and len(w) > 2}
